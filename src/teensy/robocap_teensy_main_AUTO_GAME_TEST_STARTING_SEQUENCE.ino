...
static constexpr bool TEST_DEADMAN_ENABLE = true;    // stops manual drive if motor commands stop
static constexpr uint32_t TEST_DEADMAN_MS = 450;
bool testEnteredFromFault = false;
// Motor and dribbler deadman timers are intentionally SEPARATE:
//   lastTestMotorCmdMs    -> reset only by OMNI/MOTOR; motors killed on timeout.
//   lastTestDribblerCmdMs -> reset only by DRIBBLER; dribbler is NOT killed by the
//                            motor deadman. The ESP hold loop re-sends every 60 ms,
//                            so this never times out while the web button is ON.
//                            Dribbler stops only when pct=0 or ESTOP arrives.
uint32_t lastTestMotorCmdMs    = 0;
uint32_t lastTestDribblerCmdMs = 0;
static inline void markTestMotorCmd()    { lastTestMotorCmdMs    = millis(); }
static inline void markTestDribblerCmd() { lastTestDribblerCmdMs = millis(); }
...
// Movement vector is rotated +22.5 degrees to fine-tune robot-forward alignment.
...
// TEST-only helper: snap joystick/vector commands to D-pad movement.
// Translation can only be forward, backward, left, or right.
// Rotation is still passed through normally. The snapped vector then uses omniDrive(),
// so it still gets the +22.5 degree offset above.
void omniDriveTestDpad(float vx, float vy, float omega)
{
  ...
  // D-pad snap: choose stronger axis
  // if (fabs(vx) > fabs(vy)) {
  //   if (vx > 0) {
  //     omniDrive(1, 0, 0);   // right
  //   } else {
  //     omniDrive(-1, 0, 0);  // left
  //   }
  // } else {
  //   if (vy > 0) {
  //     omniDrive(0, 1, 0);   // forward
  //   } else {
  //     omniDrive(0, -1, 0);  // backward
  //   }
  // }
}
...
void testDeadmanUpdate()
{
  if (!TEST_DEADMAN_ENABLE || sysState != S_TEST)
    return;
  // Only kill motors on timeout — dribbler has its own separate timer and is
  // intentionally NOT stopped here (it is controlled by the ESP hold loop).
  if (lastTestMotorCmdMs && millis() - lastTestMotorCmdMs > TEST_DEADMAN_MS)
  {
    motorKill();
    analogWrite(PIN_KICKER, 0);
    lastTestMotorCmdMs = 0;
  }
}
...
// Return the shortest signed difference from "from" to "to" in degrees.
// Works correctly across the 0/360 wrap, e.g. 350 -> 10 is +20 deg.
float angleDiffDeg(float from, float to)
{
  float diff = to - from;
...
void driveTimed(uint32_t ms, float vx, float vy, float omega, uint8_t dribblerPwm)
{
  timer = 0;
  while (timer < ms)
  {
    keepCommsAndCompassAlive();
    dribblerSet(dribblerPwm);
    omniDrive(vx, vy, omega);
  }
  omniDrive(0.0f, 0.0f, 0.0f);
}

void turnByCompass(float degrees, float power, uint32_t timeoutMs)
{
  if (!bnoOK)
  {
    Serial.println("[AUTO] BNO not OK: gyro turn skipped");
    espSendAscii("LOG:AUTO_GYRO_NOT_OK");
    omniDrive(0.0f, 0.0f, 0.0f);
    return;
  }
  ...
  float targetAbs = fabsf(degrees);
  float dir = (degrees >= 0.0f) ? 1.0f : -1.0f;

  timer = 0;
  while (fabsf(angleDiffDeg(startHeading, heading)) < targetAbs && timer < timeoutMs)
  {
    keepCommsAndCompassAlive();
    omniDrive(0.0f, 0.0f, dir * power);
  }

  omniDrive(0.0f, 0.0f, 0.0f);

  Serial.printf("[AUTO] gyro turn %.1f deg done: start=%.1f now=%.1f turned=%.1f timeout=%d\n",
                degrees, startHeading, heading, angleDiffDeg(startHeading, heading), timer >= timeoutMs);
}
...
void handleAsciiFromEsp(int portIdx, const char *line)
{
  ...
  if (!strcmp(line, "TEST:ON"))
  {
    ...
    if (sysState == S_READY || allowFaultTest)
    {
      ...
      lastTestMotorCmdMs = 0; lastTestDribblerCmdMs = 0;
      sysState = S_TEST;
  ...
  if (!strcmp(line, "TEST:OFF") || !strcmp(line, "EXIT:READY"))
  {
    motorKill();
    dribblerSet(0);
    analogWrite(PIN_KICKER, 0);
  ...
  if (!strcmp(line, "GAME:ON") || !strcmp(line, "EXIT:GAME"))
  {
  ...
  { // vx vy r (-100..100), TEST snaps translation to D-pad directions
    omniDriveTestDpad(a / 100.0f, b / 100.0f, c / 100.0f);
    markTestMotorCmd();
    espSendAscii("ACK:OMNI_DPAD");
    return;
  }
  ...
  {
    int pct = constrain(a, 0, 100);
    int pwm = map(pct, 0, 100, 0, 255);
    dribblerSet((uint8_t)pwm);
    markTestDribblerCmd();

    char ack[32];
    snprintf(ack, sizeof(ack), "ACK:DRIBBLER:%d", pct);
    espSendAscii(ack);
    return;
  }
...
void loop()
{
  ...
  if (runEdge)
  {
    ...
    // TEST is sticky now: RCJ pin / run-signal edges must NOT leave TEST.
    // The only ways out of TEST are explicit web/app button commands:
    //   TEST:OFF -> READY/FAULT bypass return
    //   GAME:ON  -> GAME
    // This prevents TEST joystick/dribbler control from being interrupted by
    // pin-9 noise or a competition start signal.
#if 1
    if (sysState == S_TEST)
    {
      espSendAscii("LOG:RCJ_EDGE_IGNORED_IN_TEST");
      return;
    }
  ...
  case S_GAME:
  {
    ...
    const uint8_t DRIBBLER_55 = map(55, 0, 100, 0, 255);

    // 1. Dribbler on at 55%, small left/side move by time.
    dribblerSet(DRIBBLER_55);
    driveTimed(200, 0.0f, 0.7f, 0.0f, DRIBBLER_55);
    delay(100);

    // 2. Turn 90 degrees using the BNO/compass heading.
    // If this turns the wrong way, change +90.0f to -90.0f.
    turnByCompass(90.0f, 0.4f, 2500);

    // 3. Move ~60cm by time.
    driveTimed(2000, 0.0f, 0.8f, 0.0f, DRIBBLER_55);
    delay(550);

    // 4. Second turn: about 15 degrees using gyro/compass.
    // If this turns the wrong way, change +15.0f to -15.0f.
    turnByCompass(15.0f, 0.35f, 1200);

    // 5. Dribbler 100%.
    dribblerSet(255);
    delay(2500);

    // 6. This is rotation, not strafe, because omega is the 3rd parameter.
    driveTimed(90, 0.0f, 0.0f, -1.0f, 255);
    delay(10000);
